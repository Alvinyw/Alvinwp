...
<link type="text/css" rel="shortcut icon" href="<?php echo get_template_directory_uri();?>/img/favicon.ico" />
<script src="<?php echo get_template_directory_uri();?>/js/jq.snow.js"></script>
    <!--下面是调用方法和参数说明-->
	<script>
	$(function(){
		//$.fn.snow({ 
...
<div class="uk-width-small-1-1 uk-width-medium-1-4 uk-width-large-1-5">
    <div id="head" data-uk-sticky="{boundary: true}">
        <div class="uk-panel">
            <div class="tx pr">
                <div id="festival"><img class="img-responsive" src="<?php echo get_template_directory_uri();?>/img/img-icon/cur-festival.png" alt="Festival"/></div>
                <?php if(of_get('avatar') != ''){?>
                <img src="<?php echo of_get('avatar');?>" />
                <?php }else{?>
                <img id="myHead" src="<?php echo get_template_directory_uri();?>/img/default.jpg" />
                <?php }?>
            </div>
            ...
            <div class="my uk-grid-collapse uk-grid uk-grid-width-1-3">
                <div> <span>
                    ...
                    </span> <span><i class="uk-icon-file-text"></i></span> <a href="<?php the_permalink(of_get('page_archive')); ?>" title="文章" data-uk-tooltip="{pos:'bottom'}"></a> </div>
                <div> <span><?php echo $count_categories = wp_count_terms('category'); ?></span> <span><i class="uk-icon-folder"></i></span> <a href="<?php the_permalink(of_get('page_category')); ?>" title="分类" data-uk-tooltip="{pos:'bottom'}"></a> </div>
                <div> <span><?php echo $count_tags = wp_count_terms('post_tag'); ?></span> <span><i class="uk-icon-tags"></i></span> <a href="<?php the_permalink(of_get('page_tag')); ?>" title="标签" data-uk-tooltip="{pos:'bottom'}"></a> </div>
            </div>
            <?php
